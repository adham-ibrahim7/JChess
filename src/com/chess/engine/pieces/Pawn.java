		...
		for (final int currentCandidateOffset : CANDIDATE_MOVE_COORDINATE) {
			...
			if (currentCandidateOffset == 8 && !board.getTile(candidateDestinationCoordinate).isTileOccupied()) {
				...
				if (!board.getTile(behindCandidateDestinationCoordinate).isTileOccupied()
					...
					!(BoardUtils.EIGTH_RANK[this.piecePosition] && this.pieceAlliance.isWhite()) &&
					!(BoardUtils.FIRST_RANK[this.piecePosition] && this.pieceAlliance.isBlack())) {
				if (board.getTile(candidateDestinationCoordinate).isTileOccupied()) {
					...
					!(BoardUtils.EIGTH_RANK[this.piecePosition] && this.pieceAlliance.isBlack()) &&
					!(BoardUtils.FIRST_RANK[this.piecePosition] && this.pieceAlliance.isWhite())) {
				if (board.getTile(candidateDestinationCoordinate).isTileOccupied()) {
					final Piece pieceOnCandidate = board.getTile(candidateDestinationCoordinate).getPiece();
					if (this.pieceAlliance != pieceOnCandidate.getPieceAlliance()) {
						//TODO more to do here
